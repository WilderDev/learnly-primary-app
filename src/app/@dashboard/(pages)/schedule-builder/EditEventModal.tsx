...
import { ICalendarEvent, TEvent } from '@/assets/typescript/schedule';
...
// * Component
export default function EditEventModal({ isOpen, close, event }: IProps) {
  // * Context / Hooks
  ...
  const [type, setType] = useState<TEvent>(event.type || 'LESSON');
  const [datetime, setDatetime] = useState<Date | null>(
    new Date(event.datetime) || null,
  );
  const [lengthInMin, setLengthInMin] = useState(event.lengthInMin || 60);
  ...
  // * Requests / Mutations
  const { mutate, isLoading } = useRequest(editEvent, {
    onSuccess: (data) => {
      if (data.ok) {
        setDate(new Date());
  ...
  return (
    ...
    >
      <Modal.Header title={`Update Event: ${event.name}`} />
