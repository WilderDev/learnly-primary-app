...
export function getIndexColors(idx: number) {
  const index = idx as keyof typeof indexColors;

  return indexColors[index] || indexColors[100];
}
...
// * User Status Colors
// User Status Helper Function
export function getUserStatusColor(userStatus: string) {
  const status = userStatus
    .toUpperCase()
    .split(' ')
    .join('_') as keyof typeof userStatusColors;

  return userStatusColors[status] || userStatusColors.DEFAULT;
}

// User Status Enum Colors
